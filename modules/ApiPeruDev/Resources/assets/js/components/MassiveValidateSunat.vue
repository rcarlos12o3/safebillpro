<template>
    <div>
        <el-dialog :title="title"
            ...
            <div class="row" v-loading="loading_submit" :element-loading-text="loading_text">
                <div class="col-md-12 mb-3">
                    <el-alert
                        title="Validación directa con SUNAT"
                        type="info"
                        :closable="false"
                        show-icon>
                        <template slot="title">
                            <i class="fa fa-info-circle"></i> Validación directa con SUNAT (sin costo)
                        </template>
                        Este servicio consulta directamente a SUNAT usando sus credenciales configuradas.
                        Requiere Client ID y Client Secret configurados en Empresa > Consulta integrada de CPE.
                    </el-alert>
                </div>
                <div class="col-md-12">
                ...
                <div class="col-md-12 mt-4 text-right">
                    ...
                    <el-button type="primary" @click.prevent="onSubmit" :loading="loading_submit">
                        <i class="fa fa-check"></i> Buscar y Validar
                    </el-button>
                </div>
...
export default {
    name: 'MassiveValidateSunat',
    props: ['showDialogValidate'],
    data() {
        return {
            loading: false,
            loading_submit: false,
            loading_text: 'Validando documentos con SUNAT... Esto puede tardar varios minutos.',
            resource: 'apiperudev/massive_validate_sunat',
            title: '',
            errors: {},
            form: {},
            document_types: [],
    ...
    created() {
        this.initTables();
        this.title = 'Validación masiva con SUNAT (directo)'
    },
    methods: {
        initTables() {
            ...
            await this.$http.post(`/${this.resource}`, this.form)
                .then((response) => {
                    ...
                    if (res.success) {
                        ...
                        if (res.data) {
                            const details = `
                                Documentos consultados: ${res.data.total_documents || 0}
                                Documentos actualizados: ${res.data.total_updated || 0}
                                Errores: ${res.data.total_errors || 0}
                            `;
                            this.$notify({
                ...
                .catch((error) => {
                    if (error.response && error.response.data) {
                        this.$message.error(error.response.data.message || 'Error en la validación');
                    } else {
