    ...
    @foreach($document->items as $row)
        <tr>
            ...
            <td class="text-left desc-9 align-top font-bold">
                ...
                @php
                    $itemId = isset($row->item->id) ? $row->item->id : (isset($row->item_id) ? $row->item_id : null);
                    $itemModel = $itemId ? \App\Models\Tenant\Item::find($itemId) : null;
                    $itemSizes = $itemModel ? $itemModel->getItemSize() : collect();
                    $itemSize = $itemSizes->first();
                @endphp
                @if($itemSize && isset($itemSize->cat_item_size))
                    <br/><span style="font-size: 9px; font-weight: bold;">Talla: {{ $itemSize->cat_item_size->name }}</span>
                @endif
